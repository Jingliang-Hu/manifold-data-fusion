...
HSIData = cat(1,labeledHSIData,unLabeledHSIData);
SE1Data = cat(1,labeledPOLData,unLabeledPOLData);
Z = [HSIData',zeros(size(HSIData,2),size(SE1Data,1));zeros(size(SE1Data,2),size(HSIData,1)),SE1Data'];

bin = 5:5:55;
mu = 0.5:0.5:3;
dim = 5:5:50;

oa = zeros(length(mu),length(bin),length(dim),5);



for cv_bin = 1:length(bin)
    [ T_ul ] = mapperTopo_parameter_pc( HSIData, bin(cv_bin) );
    [ T_br ] = mapperTopo_parameter_pc( SE1Data, bin(cv_bin) );
...
%     figure,imshow(T);
    ...
    for cv_mu = 1:length(mu)
        [ map1,map2 ] = mani_version_2( S,D,T,Z,mu(cv_mu),size(HSIData,2),size(SE1Data,2) );
        ...
        parfor cv_dim = 1:length(dim)
            HSIFeatProj = HSIFeat * map1(:,1:dim(cv_dim));
            SE1FeatProj = PolFeat * map2(:,1:dim(cv_dim));

            disp('')
            disp('---------------------')      

            feat = cat(2,HSIFeatProj,SE1FeatProj);            
            for i = 1:size(feat,2)
        ...
        oa(cv_mu,cv_bin,:,1) = oa_knn;
        oa(cv_mu,cv_bin,:,2) = oa_lsvm;
        oa(cv_mu,cv_bin,:,3) = oa_ksvm;
        oa(cv_mu,cv_bin,:,4) = oa_rf;
        oa(cv_mu,cv_bin,:,5) = oa_ccf;

    end
end
save MA_MIMA_PC oa dim mu bin
