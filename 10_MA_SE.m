...
oa = zeros(length(mu),length(k),length(dim),5);
...
for cv_mu = 1:length(mu)
    [ map1,map2 ] = mani_version_2( S,D,T,Z,mu(cv_mu),size(LS8Data,2),size(SE1Data,2) );
    ...
    oa(cv_mu,cv_k,:,1) = oa_knn;
    oa(cv_mu,cv_k,:,2) = oa_lsvm;
    oa(cv_mu,cv_k,:,3) = oa_ksvm;
    oa(cv_mu,cv_k,:,4) = oa_rf;
    oa(cv_mu,cv_k,:,5) = oa_ccf;
